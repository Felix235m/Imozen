...
export function TaskCard({
  ...
  // Get lead_type with fallback lookup from localStorage (client-side only)
  const getLeadType = (): 'Buyer' | 'Seller' => {
    if (!isHydrated) {
      // During hydration, use task data or default
  ...
  // Get lead priority with fallback lookup from localStorage (client-side only)
  ...
  React.useEffect(() => {
    ...
    // Listen for visibility changes only if document is available
    ...
    return () => {
      // Clean up timeout on unmount for SSR safety
      if (timeoutId) {
        clearTimeout(timeoutId);
      }
    };
  ...
  const handleReschedule = async (newDate: Date, note: string, newTime: string) => {
    ...
    try {
      ...
      console.warn('Failed to store rollback data:', error);
    }
  ...
  const handleCancellationSuccess = async (responseData: any, cancellationData: any) => {
    ...
    if (responseData.current_note || (responseData.notes && responseData.notes.length > 0)) {
      try {
        ...
        if (noteProcessingResult.success && noteProcessingResult.notesProcessed > 0) {
          ...
          console.log(`📝 Successfully processed ${noteProcessingResult.notesProcessed} new notes for lead ${cancellationData.lead_id}`);
        }
      } catch (error) {
  ...
  const handleCancel = async (note: string, nextFollowUpDate?: Date, scheduleNext?: boolean) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Failed to store pending cancellation data:', error);
      }
      ...
      try {
        ...
        console.warn('Failed to store cancellation rollback data:', error);
      }
      ...
      try {
        rollbackData = localStorage.getItem('cancellation_rollback_data');
      } catch (error) {
        console.warn('Failed to get cancellation rollback data:', error);
      }
      ...
      const cancellationNotificationId = createCancellationProgressNotification(task.id, agentName, note || '', nextFollowUpDate);
      await handleCancellationFailure(error, task.id, cancellationNotificationId);
    } finally {
  ...
  const handleMarkDone = async (note: string, nextFollowUpDate?: Date) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Failed to store mark done rollback data:', error);
      }
      ...
      const completionEvent = createFollowUpCompletedEvent(task.id, agentName, note || '');
      ...
      let storedRollbackData: string | null = null;
      try {
        storedRollbackData = localStorage.getItem('mark_done_rollback_data');
      } catch (error) {
        console.warn('Failed to get mark done rollback data:', error);
      }
  ...
  // Prevent hydration mismatch by showing a placeholder during hydration
  if (!isHydrated) {
    return (
      <div className="shadow-sm cursor-pointer transition-all duration-300 ease-in-out mb-3 rounded-xl">
        <div className="p-4">
