...
export default function LeadDetailPage() {
  ...
  const getFieldLabel = useCallback((fieldKey: string): string => {
    const fieldMap: Record<string, string> = {
      'name': t.leads.name,
      'Contact Phone': t.leads.fieldLabels.contactPhone,
      'Contact Email': t.leads.fieldLabels.contactEmail,
      'Contact Language': t.leads.fieldLabels.contactLanguage,
      'Property.Type': t.leads.fieldLabels.propertyType,
      'Property.Budget': t.leads.fieldLabels.propertyBudget,
      'Property.Bedrooms': t.leads.fieldLabels.propertyBedrooms,
      'Property.Locations': t.leads.fieldLabels.propertyLocations,
    };
  ...
  const prepareSaveChanges = useCallback(() => {
    ...
    // Create the current lead data with all updated contact fields
    ...
    console.log('🔍 DEBUG: currentLeadData contact:', currentLeadData.contact);
    ...
    const findChanges = (
      ...
      for (const key in original) {
        ...
        // Special handling for contact object to ensure proper comparison of all fields
        if (key === 'contact' && typeof originalValue === 'object' && originalValue !== null && typeof currentValue === 'object' && currentValue !== null) {
          // Compare each field in the contact object
          for (const contactField in originalValue) {
            ...
            if (JSON.stringify(originalContactValue) !== JSON.stringify(currentContactValue)) {
              console.log(`🔍 DEBUG: Contact ${contactField} change detected:`, {
                original: originalContactValue,
                current: currentContactValue
              });
              
              const fieldKey = contactField === 'phone' ? 'Contact Phone' :
          ...
          // Skip deeper comparison for contact object since we handled all fields specially
          continue;
        }
        ...
        if (typeof originalValue === 'object' && originalValue !== null && !Array.isArray(originalValue)) {
            ...
            const autoGeneratedFieldName = fieldName.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
            changes.push({
                field: getFieldLabel(autoGeneratedFieldName),
    ...
    const changes = findChanges(originalLead, currentLeadData);
  ...
  }, [lead, originalLead, phoneCountryCode, phoneNumber, toast, getFieldLabel]);
  ...
  const handleSaveLeadChanges = async () => {
    ...
    try {
      ...
      console.log('🔍 DEBUG: Saving lead with data:', updatedLeadData);
      ...
      // Update component state to reflect the saved changes
        ...
        try {
            ...
            if (result.current_note || (result.notes && result.notes.length > 0)) {
                try {
                    ...
                    console.log(`📝 [LEAD_NOTES] Webhook note processing completed:`, noteProcessingResult);
                    
                    // Update UI with processed notes from localStorage
                    const processedNotes = localStorageManager.getNotes(lead.lead_id) || [];
                    if (processedNotes.length > 0) {
                        // Sort by created_at descending
                        const sortedNotes = [...processedNotes].sort((a: any, b: any) =>
                            new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
                        );
                    ...
                    console.error('❌ [LEAD_NOTES] Error processing webhook notes:', error);
                }
            }
